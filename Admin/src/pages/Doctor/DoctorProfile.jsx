...
const DoctorProfile = () => {
    ...
    try {
        ...
        if (data.success) {
            ...
            window.location.reload(); // Redirect or handle accordingly
        } else {
            toast.error(data.message);
        }
    } catch (error) {
                ...
                <div className='flex-1 border border-stone-100 rounded-lg p-8 py-7 bg-white'>
                    ...
                    <div className='flex gap-2 mt-5'>
                        {isEdit ? (
                            ...
                            <button onClick={() => setIsEdit(prev => !prev)} className='px-4 py-1 border border-primary text-sm rounded-full hover:bg-primary hover:text-white transition-all'>Edit</button>
                        )}
                        <button onClick={deleteProfile} className='px-4 py-1 border border-red-500 text-sm rounded-full hover:bg-red-500 hover:text-white transition-all'>Delete Profile</button>
                    </div>
                </div>
            </div>
        </div>
